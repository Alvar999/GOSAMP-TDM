...
TPPlayer(playerid,Float:x,Float:y,Float:z){
	SetPlayerPos(playerid,x,y,z);
	TeleP[playerid] = 1;
}
TPFindZ(playerid,Float:x,Float:y,Float:z){
...
NationSel_HandleNationSelection(playerid)
{
    ...
    new string128[128], name[128], ip[128];
    GetPlayerName(playerid,name,24);
    GetPlayerIp(playerid,ip,24);
    format(string128,sizeof(string128),"{ff9410}Luu y: %s (ID: %i) vua dang ky tham gia may chu", name, ip);
    format(string128,sizeof(string128),"{ff3210}Hay Kiem Tra /whitelist va duyet");
    ABroadCast(COLOR_YELLOW , string128, 1);
//	if(PlayerInfo[playerid][pXacNhanDangKy] == 0) { // Kiem tra xac nhan

//		PlayerInfo[playerid][pReg] = 0; // Reset Register

//		ShowPlayerDialog(playerid, 1,DIALOG_STYLE_MSGBOX, "Thong Bao","Ten nhan vat cua ban da duoc gui den BQT!\nVui long cho xac nhan de duoc vao game!\nNeu cam thay duyet lau hay lien he doi ngu Ho Tro VIen","Ok","");
...
//	}
	...
	SendClientMessage(playerid, -1, "{00FF00}GOSAMP.TOP {00ffd7}TeamDeathMatch");
	SendClientMessage(playerid, -1, "Map:{00FF00}Los Santos");
	SendClientMessage(playerid, -1, "{00ffd7}Ban Duoc Tang 1 Voucher Car Va 1 Voucher Vip");
	SendClientMessage(playerid, -1, "{ff00ff}SERVER TEAMDEATHMATCH Noi Khong Voi Hacking/Cheat");
	SetCameraBehindPlayer(playerid);
	SetPlayerVirtualWorld(playerid, 0);
	PlayerInfo[playerid][pCash] += 100000;
	PlayerInfo[playerid][pSpeedo] += 1;
	PlayerInfo[playerid][pVehVoucher] += 10;
	PlayerInfo[playerid][pConnectHours] += 2;
	PlayerInfo[playerid][pCredits] += 100;
	PlayerInfo[playerid][pVehVoucher] += 1;
	PlayerInfo[playerid][pGVIPVoucher] += 20;
	SetPlayerPos(playerid, -1809.8002,-1571.7952,13.4736);
	//ShowPlayerDialog(playerid, QUOCTICH, DIALOG_STYLE_LIST, "{ffff00}Ban Muon Cu Song O Thanh Pho Nao", "{5fffd7}Los Santos\n{5fff00}San Ferrio", "Chon", "Thoat");
	new str[2460], str1[1280], str2[1280], str3[1280], str4[1280], str5[1280], str6[1280], names[1280];
	format(str1, sizeof(str), "{ffffff}=========================== {ffd905}GOSAMP.TOP CHAO BAN !{ffffff} ===========================", GetPlayerNameEx(playerid));
    format(str2, sizeof(str), "{ffd905}Chao mung ban da den voi GOSAMP.TOP | Phien ban:"SERVER_GM_TEXT"");
    format(str3, sizeof(str), "{ffffff}Lam Ho So Can Cuoc De Co The Bat Dau Cong Viec");
    format(str4, sizeof(str), "{FFFFFF}Neu Ban Gap Kho Khan Su dung lenh: {228B22}/ls{FFFFFF}, {228B22}/yeucautrogiup");
    format(str5, sizeof(str), "{FFFFFF}Su dung lenh: {00ff00}/phieucuatoi{FFFFFF}, de lay vip.");
	format(str6, sizeof(str), "{b22222}Lenh cho newbie  : {00ff00}[/fixketvw] , [/sf] {FFFFFF} .");
    format(str, sizeof(str), "%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s", str1,str2,str3,str4,str5,str6);
    format(names, sizeof(names), "Chao Mung");
	ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, names, str, "Dong Y", "");
	SetCameraBehindPlayer(playerid);
	SetPlayerVirtualWorld(playerid, 0);
	SetPlayerPos(playerid, 1809.8002,-1571.7952,13.4736); //spawn newbie chá»— nÃ y lÃ  khi Ä‘Äƒng kÃ­ nha ae Ä‘á»•i Ä‘i nhe ko lÃ  bá»‹ spawn ra chá»— nhÆ° mÃ¬nh ..tÃ¹y ae thÃ´i.
	PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
	GetPlayerIp(playerid, ip, sizeof(ip));
	format(str, sizeof(str), "{AA3333}[DANG KY]{FFFFFF}: %s (IP: %s) da dang ky tham gia vao may chu.", GetPlayerNameEx(playerid), ip);
	ABroadCast(-1, str, 2);
 }
...
TPPlayerObjectOffset(objectid, playerid, Float:offset_x, Float:offset_y, Float:offset_z)
{
	new Float:object_px,
        Float:object_py,
        Float:object_pz,
	...
	if(PokerTable[tableid][pkrPlayers] < PokerTable[tableid][pkrLimit])
	{
		...
		for(new s; s < 6; s++) {
			if(PokerTable[tableid][pkrSlot][s] == -1) {
				...
				ApplyAnimation(playerid, "CASINO", "cards_out", 4.1, 0, 1, 1, 1, 1, 1);
				TogglePlayerControllable(playerid, 0);
				TPPlayerObjectOffset(PokerTable[tableid][pkrObjectID], playerid, PokerTableMiscObjOffsets[s][0], PokerTableMiscObjOffsets[s][1], PokerTableMiscObjOffsets[s][2]);
...
LeavePokerTable(playerid)
{
	...
	SetPlayerInterior(playerid, PokerTable[tableid][pkrInt]);
	SetPlayerVirtualWorld(playerid, PokerTable[tableid][pkrVW]);
	TPPlayer(playerid, GetPVarFloat(playerid, "pkrTableJoinX"), GetPVarFloat(playerid, "pkrTableJoinY"), GetPVarFloat(playerid, "pkrTableJoinZ")+0.1);
	SetCameraBehindPlayer(playerid);
	TogglePlayerControllable(playerid, 1);
...
ShowCasinoGamesMenu(playerid, dialogid)
{
	switch(dialogid)
	{
		...
		{
			return ShowPlayerDialog(playerid, DIALOG_CGAMESCREDITS, DIALOG_STYLE_MSGBOX, "{FFFFFF}Casino Games - (Creadits)", "{FFFFFF}Chinh sua boi: Laur.", "Quay Lai", "");
		}
...
IsPlayerInRangeOfPlayer(playerid, giveplayerid, Float: radius) {

	new
		Float:Floats[3];

	GetPlayerPos(giveplayerid, Floats[0], Floats[1], Floats[2]);
	return IsPlayerInRangeOfPoint(playerid, radius, Floats[0], Floats[1], Floats[2]);
}
...
public InitiateGamemode()
{
    ...
    // Anti Hack Fly
    //SetTimerEx("AntiHackSpeedCar",5000,0,"i",playerid); // 5000 = 5 giay
    //SetTimerEx("AntiHackFly",5000,0,"i",playerid);
    //SetTimerEx("AntiSpamCar",5000,0,"i",playerid);
    // Bai toan
    LoadGame();
	...
	print("\n-------------------------------------------");
	print("GOSAMP\n");
	print("GOSAMP Khong Hacking Game 2024");
	print("GOSAMP Open PC/MB 2024");
	print("-------------------------------------------\n");
...
public SetCamBack(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		...
		TPPlayer(playerid, -1863.15, -21.6598, 1060.15); // Warp the player
...
public MoveEMS(playerid)
{
	...
	TPPlayer(GetPVarInt(playerid, "MovingStretcher"), mX, mY, mZ);
...
public SendEMSQueue(playerid,type)
{
	...
	switch (type)
	{
		case 1:
		{
			...
			TPPlayer(playerid, GetPVarFloat(playerid,"MedicX"), GetPVarFloat(playerid,"MedicY"), GetPVarFloat(playerid,"MedicZ"));
...
public rccam(playerid)
{
	DestroyVehicle(GetPVarInt(playerid, "rcveh"));
 	TPPlayer(playerid, GetPVarFloat(playerid, "rcX"), GetPVarFloat(playerid, "rcY"), GetPVarFloat(playerid, "rcZ"));
...
public cameraexpire(playerid)
{
	SetPVarInt(playerid, "cameraactive", 0);
 	...
 	{
	 	TPPlayer(playerid, GetPVarFloat(playerid, "cameraX2"), GetPVarFloat(playerid, "cameraY2"), GetPVarFloat(playerid, "cameraZ2"));
...
public PokerPulse(tableid)
{
	...
	for(new i = 0; i < 6; i++) {
		...
		if(playerid != -1) {
			...
			if(idleRandom >= 90) {
			    TPPlayerObjectOffset(PokerTable[tableid][pkrObjectID], playerid, PokerTableMiscObjOffsets[i][0], PokerTableMiscObjOffsets[i][1], PokerTableMiscObjOffsets[i][2]);
	...
	switch(GetPVarInt(iPlayer, "StreamPrep")) {
		...
		case 1: {
			...
			if(GetPlayerState(iPlayer) == PLAYER_STATE_DRIVER)
				...
				TPPlayer(iPlayer, fPosX, fPosY, fPosZ + 0.5);
		...
		default: {
			...
			if(GetPlayerState(iPlayer) == PLAYER_STATE_DRIVER)
				...
				TPPlayer(iPlayer, fPosX, fPosY, fPosZ);
...
public HealthHackCheck(playerid, giveplayerid)
{
	...
	if(playerTabbed[giveplayerid] != 0)
	{
		...
		TPPlayer(giveplayerid, HHcheckFloats[giveplayerid][2], HHcheckFloats[giveplayerid][3], HHcheckFloats[giveplayerid][4]);
...
stock GetDeath(reason)
{
	new name[50];
	switch(reason)
	{
		case 0: name = "nguoi ta danh chet";
		case 1: name = "nguoi ta danh chet";
		case 2: name = "gay golf dap vao dau";
		case 3: name = "bi xien chet";
		case 4: name = "dam vao bung bang dao";
		case 5: name = "gay bong chay dap vao dau";
		case 6: name = "cay xeng danh gay co";
		case 7: name = "gay bida chot vao trym";
		case 8: name = "bi chem chet";
		case 9: name = "cua ra lam doi bang may cua";
		case 10: name = "sextoy thong";
		case 11: name = "sextoy thong";
		case 12: name = "sextoy thong";
		case 13: name = "sextoy thong";
		case 14: name = "di ung voi bong hoa";
		case 15: name = "gay coc dap vao dau";
		case 16: name = "luu dan no bay xac";
		case 17: name = "ngop tho khi gas";
		case 18: name = "thieu song";
		case 19: name = "sung tren phuong tien ban";
		case 20: name = "sung tren phuong tien ban";
		case 22: name = "Ha guc boi sung 9MM";
		case 23: name = "Ha guc boi sung SD pistol";
		case 24: name = "Ha guc boi sung Deagle";
		case 25: name = "Ha guc boi sung Shotgun";
		case 26: name = "Ha guc boi sung Sawnoff Shotgun";
		case 27: name = "Ha guc boi sung Spas12";
		case 28: name = "Ha guc boi sung Uzi";
		case 29: name = "Ha guc boi sung MP5";
		case 30: name = "Ha guc boi sung AK47";
		case 31: name = "Ha guc boi sung M4";
		case 32: name = "Ha guc boi sung Tec";
		case 33: name = "Ha guc boi sung truong Rifle";
		case 34: name = "Am sat boi sung Sniper";
		case 35: name = "Khau RPG ban trung no banh xac";
		case 36: name = "khau RPG ban trung no banh xac";
		case 37: name = "Bi dot song boi sung phun lua";
		case 38: name = "Minigun cuc manh";
		case 49: name = "xe tong chet";
		case 50: name = "May bay chem chet";
		case 51: name = "Chet trong mot vu no";
		case 52: name = "Bi thang ngu nao do CP";
		case 53: name = "Chet duoi";
		case 54: name = "Chet ly do khong hieu";
		case 55: name = "Chet ly do khong hieu";
	}
	return name;
}
...
stock SetPlayerJoinCamera(playerid)
{
	...
	switch(randcamera)
	{
		case 1: // Gym
		{
			...
			TPPlayer(playerid,2211.1460,-1748.3909,-10.0);
		...
		case 2: // Paintball Arena
		{
			...
			TPPlayer(playerid,1283.8524,-1385.5304,-10.0);
		...
		case 3: // LSPD
		{
			...
			TPPlayer(playerid,1514.7783,-1700.2913,-10.0);
		...
		case 4: // SaC HQ (Gang HQ)
		{
			...
			TPPlayer(playerid,655.5394,-1867.2231,-10.0);
		...
		case 5: // Fishing Pier
		{
			...
			TPPlayer(playerid,370.0804,-2087.8767,-10.0);
		...
		case 6: // VIP
		{
			...
			TPPlayer(playerid,1797.3397,-1578.3440,-10.0);
		...
		case 7: // All Saints
		{
			...
			TPPlayer(playerid, 1188.4574,-1309.2242,-10.0);
...
stock ShowTutGUIBox(playerid)
{
	...
	TextDrawShowForPlayer(playerid, LoginHoclamSAMP);
...
stock HideTutGUIBox(playerid)
{
	...
	TextDrawHideForPlayer(playerid, LoginHoclamSAMP);
	...
	if(GetPVarInt(playerid, "MedicBill") == 1 && PlayerInfo[playerid][pJailTime] == 0)
	{
		switch(PlayerInfo[playerid][pHospital]) {
			case 1:
			{
				...
				TPPlayer(playerid, 1175.0586,-1324.2463,14.5938);
			...
			case 2:
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien $1,500. Chuc mot ngay tot lanh!");
				TPPlayer(playerid, 1804.0280,-1654.1016,14.5093);
			...
			case 3:
			{
				...
				TPPlayer(playerid, 1241.4888,325.9947,19.7555);
			...
			case 4:
			{
				...
				TPPlayer(playerid, -320.3253, 1049.2809, 20.3403);
				SetPlayerFacingAngle(playerid, 179.4258);
				PlayerInfo[playerid][pHospital] = 0;
				SetCameraBehindPlayer(playerid);
				TogglePlayerControllable(playerid, 1);
			...
			case 5:
			{
				...
				Misc_Save();
				TPPlayer(playerid, 2034.2269,-1404.3459,17.2617);
			...
			case 7:
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien $250. Chuc mot ngay tot lanh!!");
				TPPlayer(playerid, 2785.553955, 2394.641845, 1240.531127);
			...
			case 8:
			{
				...
				for(new i = 0; i < sizeof(HouseInfo); i++)
				{
					if(PlayerInfo[playerid][pPhousekey] == i || PlayerInfo[playerid][pPhousekey2] == i)
					{
						...
						TPPlayer(playerid,HouseInfo[i][hInteriorX],HouseInfo[i][hInteriorY],HouseInfo[i][hInteriorZ]);
			...
			case 9:
			{
				...
				new Float:X, Float:Y, Float:Z;
				GetDynamicObjectPos(Carrier[0], X, Y, Z);
				TPPlayer(playerid, (X-0.377671),(Y-10.917018),11.6986);
			...
			case 10:
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien $250. Chuc mot ngay tot lanh!");
				TPPlayer(playerid, -1514.809204, 2526.305175, 55.759651);
			...
			case 11:
			{
				...
				TPPlayer(playerid, -1680.8573, 284.6186, 7.1875);
				SetCameraBehindPlayer(playerid);
				PlayerInfo[playerid][pHospital] = 0;
				SetCameraBehindPlayer(playerid);
				TogglePlayerControllable(playerid, 1);
			...
			case 12:
			{
				...
				SetPlayerHealth(playerid, 50.0);
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "DOC: Hoa don cua ban duoc chi tra mien phi, chuc mot ngay vui ve!");
				TPPlayer(playerid, 1607.4916, 1817.4746, 10.8203);
				SetCameraBehindPlayer(playerid);
				PlayerInfo[playerid][pHospital] = 0;
				SetCameraBehindPlayer(playerid);
				TogglePlayerControllable(playerid, 1);
			...
			case 13: //Famed Lounge
			{
				...
				SetPlayerHealth(playerid, 50.0);
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "DOC: Hoa don cua ban duoc chi tra mien phi, chuc mot ngay vui ve!");
				TPPlayer(playerid, 914.8001, 1427.6847, -81.1762);
				SetCameraBehindPlayer(playerid);
				PlayerInfo[playerid][pHospital] = 0;
				SetCameraBehindPlayer(playerid);
				TogglePlayerControllable(playerid, 1);
			...
			case 14: //DeMorgan
			{
				...
				SetPlayerHealth(playerid, 50.0);
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "DOC: Hoa don cua ban duoc chi tra mien phi, chuc mot ngay vui ve!");
				TPPlayer(playerid, 230.8369, 1980.1620, 17.6406);
				SetPlayerFacingAngle(playerid, 0);
				PlayerInfo[playerid][pHospital] = 0;
				SetCameraBehindPlayer(playerid);
				TogglePlayerControllable(playerid, 1);
			...
			case 15: //TR - Bayside
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien $500. Chuc mot ngay tot lanh!");
				TPPlayer(playerid, -2486.7124,2234.5493,4.8438);
...
stock SetPlayerSpawn(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		...
		if(HungerPlayerInfo[playerid][hgInEvent] == 1)
		{
			if(hgActive > 0)
			{
				if(hgPlayerCount == 3)
				{
					...
					TPPlayer(playerid, HungerPlayerInfo[playerid][hgLastPosition][0], HungerPlayerInfo[playerid][hgLastPosition][1], HungerPlayerInfo[playerid][hgLastPosition][2]);
				...
				else if(hgPlayerCount == 2)
				{
					...
					for(new i = 0; i < MAX_PLAYERS; i++)
					{
						if(HungerPlayerInfo[i][hgInEvent] == 1)
						{
							...
							TPPlayer(i, HungerPlayerInfo[i][hgLastPosition][0], HungerPlayerInfo[i][hgLastPosition][1], HungerPlayerInfo[i][hgLastPosition][2]);
		...
		if(GetPVarType(playerid, "SpecOff"))
		{
			...
			TPPlayer(playerid, GetPVarFloat(playerid, "SpecPosX"), GetPVarFloat(playerid, "SpecPosY"), GetPVarFloat(playerid, "SpecPosZ"));
		...
		if(PlayerInfo[playerid][pTut] == 0)
		{
			...
			SetPlayerInterior(playerid, 0);
			Streamer_UpdateEx(playerid, 2229.4968,-1722.0701,13.5625);
			TPPlayer(playerid, 2229.4968,-1722.0701,-10.0);
			SetPlayerCameraPos(playerid, 2211.1460,-1748.3909,29.3744);
			SetPlayerCameraLookAt(playerid, 2229.4968,-1722.0701,13.5625);
		...
		if(PlayerInfo[playerid][pBeingSentenced] > 0)
		{
			...
			TPPlayer(playerid, WarrantJail[rand][0], WarrantJail[rand][1], WarrantJail[rand][2]);
		...
		if(PlayerInfo[playerid][pJailTime] > 0)
		{
		    if(strfind(PlayerInfo[playerid][pPrisonReason], "[IC]", true) != -1)
		    {
    			...
    			TPPlayer(playerid, DocPrison[rand][0], DocPrison[rand][1], DocPrison[rand][2]);
		    ...
		    else if(strfind(PlayerInfo[playerid][pPrisonReason], "[ISOLATE]", true) != -1)
		    {
				...
				SetPlayerInterior(playerid, 10);
				PlayerInfo[playerid][pInt] = 0;
				TPPlayer(playerid, -2095.3391, -215.8563, 978.8315);
				SetPlayerSkin(playerid, 50);
				SetPlayerHealth(playerid, 100);
				SetPlayerColor(playerid, TEAM_ORANGE_COLOR);
				DeletePVar(playerid, "Injured");
		...
		if(GetPVarInt(playerid, "EventToken") == 1)
		{
			for(new i; i < sizeof(EventKernel[EventStaff]); i++)
			{
				if(EventKernel[EventStaff][i] == playerid)
				{
					...
					TPPlayer(playerid,EventFloats[playerid][1],EventFloats[playerid][2],EventFloats[playerid][3]);
            ...
            if(EventKernel[EventType] == 4)
			{
			   	TPPlayer(playerid, EventKernel[ EventPositionX ], EventKernel[ EventPositionY ], EventKernel[ EventPositionZ ] );
 				...
 				switch (randhos)
   				{
    				case 1:
  	    			{
						...
						SetPlayerCameraPos(playerid,1188.4574,-1309.2242,13.5625+6.0);
						SetPlayerCameraLookAt(playerid,1175.5581,-1324.7922,18.1610);
						TPPlayer(playerid, 1188.4574,-1309.2242,10.5625); // Warp the player
    	    		...
    	    		case 2:
	    	    	{
						...
						SetPlayerCameraPos(playerid,1999.5308,-1449.3281,13.5594+6.0);
						SetPlayerCameraLookAt(playerid,2036.2179,-1410.3223,17.1641);
						TPPlayer(playerid, 1999.5308,-1449.3281,10.5594);
						PlayerInfo[playerid][pHospital] = 2;
   					}
   				}
			}
		...
		{
			TPPlayer(playerid,PlayerInfo[playerid][pPos_x],PlayerInfo[playerid][pPos_y],PlayerInfo[playerid][pPos_z]);
			//PlayerInfo[playerid][pInterior] = PlayerInfo[playerid][pInt];
		...
		else
		{
			...
			else if(PlayerInfo[playerid][pInsurance] == 3)
			{
				...
				SetPlayerCameraPos(playerid,1248.4147,338.8385,19.4063+6.0);
				SetPlayerCameraLookAt(playerid,1241.4449,326.3389,19.7555);
				TPPlayer(playerid, 1248.4147,338.8385,19.4063);
				PlayerInfo[playerid][pHospital] = 3;
			}
   			else if(PlayerInfo[playerid][pInsurance] == 4)
			{
				...
				SetPlayerCameraPos(playerid,-314.0242,1060.7919,19.5938+6.0);
				SetPlayerCameraLookAt(playerid,-320.0992,1049.0341,20.3403);
				TPPlayer(playerid, -314.0242,1060.7919,19.5938);
				PlayerInfo[playerid][pHospital] = 4;
			}
			else if(PlayerInfo[playerid][pInsurance] == 5)
			{
				...
				SetPlayerCameraPos(playerid,-2571.2766,558.7813,68.1754);
				SetPlayerCameraLookAt(playerid,-2619.2883,596.2850,49.0966);
				TPPlayer(playerid, -2653.6685,626.6485,4.8930);
				PlayerInfo[playerid][pHospital] = 5;
			}
			else if(PlayerInfo[playerid][pInsurance] == 6)
			{
				SetPlayerCameraPos(playerid,2787.102050, 2392.162841, 1243.898681);
				SetPlayerCameraLookAt(playerid,2801.281982, 2404.575683, 1240.531127);
				TPPlayer(playerid, 2788.561523, 2387.321044, 1227.350219);
				PlayerInfo[playerid][pHospital] = 7;
			}
			else if(PlayerInfo[playerid][pInsurance] == 7)
			{
			...
			else if(PlayerInfo[playerid][pInsurance] == 8)
			{
				...
				SetPlayerCameraPos(playerid,(X-100),(Y-100),30);
				SetPlayerCameraLookAt(playerid,X, Y, Z);
				TPPlayer(playerid, (X-0.377671),(Y-10.917018),0);
				PlayerInfo[playerid][pHospital] = 9;
			}
			else if(PlayerInfo[playerid][pInsurance] == 9)
			{
				SetPlayerCameraPos(playerid, -1529.847167, 2539.394042, 62.038913);
				SetPlayerCameraLookAt(playerid, -1514.883300, 2527.161132, 55.743553);
				TPPlayer(playerid, -1514.809204, 2526.305175, 51.865501);
				PlayerInfo[playerid][pHospital] = 10;
			}
   			if(PlayerInfo[playerid][pInsurance] == 0)
			{
		...
		{
  			SetPlayerInterior(playerid,0);
			TPPlayer(playerid, 1715.1201,-1903.1711,13.5665);
...
stock TutorialStep(playerid)
{
	...
	switch(TutStep[playerid])
	{
		case 1:
		{
			...
			TPPlayer(playerid, 1457.4854,-1011.5267,-10.0);
		...
		case 2:
		{
			...
			TPPlayer(playerid, 2140.5173,-1163.9576,-10.0);
		...
		case 5:
		{
			...
			TPPlayer(playerid, 1366.1187,-1275.1248,-10.0);
		...
		case 6:
		{
			...
			TPPlayer(playerid, 2167.5542,-1673.1503,-10.0);
		...
		case 7:
		{
			...
			TPPlayer(playerid, 59.9634,-283.6652,-10.0);
		...
		case 9:
		{
			...
			SetPlayerInterior(playerid, 0);
			Streamer_UpdateEx(playerid, 1554.3381,-1675.5692,16.1953);
			TPPlayer(playerid, 1554.3381,-1675.5692,-10.0);
			SetPlayerCameraPos(playerid, 1514.7783,-1700.2913,36.7506);
			SetPlayerCameraLookAt(playerid, 1554.3381,-1675.5692,16.1953);
		}
		case 10:
		...
		case 11:
		{
			...
			TPPlayer(playerid, 643.6738,-1348.9811,-10.0);
		...
		case 12:
		{
			...
			TPPlayer(playerid, 1478.4708,-1754.7579,-10.0);
		...
		case 13:
		{
			...
			TPPlayer(playerid, -2679.5342,1639.0643,-10.0);
		...
		case 15:
		{
			...
			TPPlayer(playerid, 1315.6570,-898.7749,-10.0);
		...
		case 16:
		{
			...
			TPPlayer(playerid, 1457.1664,-1138.0111,-10.0);
		...
		case 17:
		{
			...
			TPPlayer(playerid,1656.6107,-1892.3878,-10.0);
...
stock ShowStats(playerid,targetid)
{
	...
	{
		new header[65], org[128], pnumber[20], pvtstring[128], employer[GROUP_MAX_NAME_LEN], rank[GROUP_MAX_RANK_LEN], division[GROUP_MAX_DIV_LEN];
		new sext[16], nation[24];
		if(PlayerInfo[playerid][pPnumber] == 0) pnumber = "None"; else format(pnumber, sizeof(pnumber), "%d", PlayerInfo[playerid][pPnumber]);
		...
		new totalwealth;
		totalwealth = PlayerInfo[playerid][pAccount] + GetPlayerCash(playerid);
		if(PlayerInfo[playerid][pPhousekey] != INVALID_HOUSE_ID && HouseInfo[PlayerInfo[playerid][pPhousekey]][hOwnerID] == GetPlayerSQLId(playerid)) totalwealth += HouseInfo[PlayerInfo[playerid][pPhousekey]][hSafeMoney];
		if(PlayerInfo[playerid][pPhousekey2] != INVALID_HOUSE_ID && HouseInfo[PlayerInfo[playerid][pPhousekey2]][hOwnerID] == GetPlayerSQLId(playerid)) totalwealth += HouseInfo[PlayerInfo[playerid][pPhousekey2]][hSafeMoney];
        {
			...
			if(PlayerInfo[playerid][pMember] != INVALID_GROUP_ID)
			{
				...
				format(org, sizeof(org), "{6699FF}Faction: %s (%d) / Rank: %s (%d) / Division: %s (%d) / ", employer, PlayerInfo[playerid][pMember], rank, PlayerInfo[playerid][pRank], division, PlayerInfo[playerid][pDivision]);
			}
			else if(PlayerInfo[playerid][pFMember] < INVALID_FAMILY_ID)
			{
				...
				format(org, sizeof(org), "{6699FF}Family: %s (%d) / Rank: %s (%d) / Division: %s (%d) / ", FamilyInfo[PlayerInfo[playerid][pFMember]][FamilyName], PlayerInfo[playerid][pFMember], FamilyRankInfo[PlayerInfo[playerid][pFMember]][PlayerInfo[playerid][pRank]], PlayerInfo[playerid][pRank], division, PlayerInfo[playerid][pDivision]);
			}
			...
			new staffrank[64];
			if(PlayerInfo[playerid][pHelper] > 0 || PlayerInfo[playerid][pAdmin] == 1 || (PlayerInfo[playerid][pAdmin] > 1 && PlayerInfo[playerid][pAdmin] <= PlayerInfo[playerid][pAdmin])) format(staffrank, sizeof(staffrank), "Chuc Vu: %s / ", GetStaffRank(playerid));
			else staffrank = "";
			new drank[64];
			if(PlayerInfo[playerid][pDonateRank] > 0)
			{
				switch(PlayerInfo[playerid][pDonateRank])
				{
				    case 1: drank = "{66CC33}VIP: {FFCC00}Bronze VIP";
					case 2: drank = "{66CC33}VIP: {FFCC00}Silver VIP";
					case 3: drank = "{66CC33}VIP: {FFCC00}Gold VIP";
					case 4: drank = "{66CC33}VIP: {FFCC00}Platinum VIP";
					case 5: drank = "{66CC33}VIP: {FFCC00}VIP Moderator";
				}
			}
			...
			new exp = ((50 * (PlayerInfo[playerid][pLevel]) * (PlayerInfo[playerid][pLevel]) * (PlayerInfo[playerid][pLevel]) - 150 * (PlayerInfo[playerid][pLevel]) * (PlayerInfo[playerid][pLevel]) + 600 * (PlayerInfo[playerid][pLevel])) / 5) - PlayerInfo[playerid][pXP];



	        new str[2480], str1[1280], str2[1280], str3[1280], str4[1280], str5[1280], str6[1280], str7[1280], str8[1280];
			format(header, sizeof(header), "Thong tin");
			format(str1, sizeof(str),"%s{99CCFF}Ten: %s / Level: %d / Gioi Tinh: %s / NTNS: %s / Thoi Gian Choi: %s / SDT: %s", staffrank,
	        GetPlayerNameEx(playerid),
			PlayerInfo[playerid][pLevel],
			sext,
			PlayerInfo[playerid][pBirthDate],
			number_format(PlayerInfo[playerid][pConnectHours]),
			pnumber);
	        format(str2, sizeof(str),"{66CC33}Tien: {FFCC00}$%s {99CCFF}/ {66CC33}So Tien Hien Co: {FFCC00}$%s {99CCFF}/ {66CC33}Credits: {FFCC00}%s {99CCFF} / Diem Nang Cap: {00FF33}%s {99CCFF}/ XP: {00FF33}%s {FFCC00}( So Tien Can De Nang: $%s )", number_format(GetPlayerCash(playerid)),
			number_format(totalwealth),
			number_format(PlayerInfo[playerid][pCredits]),
			number_format(PlayerInfo[playerid][gPupgrade]),
			number_format(CheckXP(exp)),
			number_format(costlevel));
			format(str3, sizeof(str),"%s%s {99CCFF}/ {FFFF00}Phu The: {66CC33}%s", org,
			drank,
			PlayerInfo[targetid][pMarriedName]);
            format(str4, sizeof(str),"{66CC33}Vat Lieu: {FFCC00}%s {99CCFF}/ {66CC33}Cay Can Sa: {FFCC00}%d {99CCFF}/ {66CC33}Ma Tuy ( Crack ): {FFCC00}%s {99CCFF}/ {66CC33}Ma Tuy ( Pot ): {FFCC00}%s {99CCFF}", number_format(PlayerInfo[playerid][pMats]),
            PlayerInfo[playerid][pCayCanSa],
			number_format(PlayerInfo[playerid][pCrack]),
			number_format(PlayerInfo[playerid][pPot]));
            format(str5, sizeof(str),"Day Thung: %s / Xi Ga: %s / Sprunk: %s / Binh Son: %s / Doi Bung: %d", number_format(PlayerInfo[playerid][pRope]),
			number_format(PlayerInfo[playerid][pCigar]),
			number_format(PlayerInfo[playerid][pSprunk]),
			number_format(PlayerInfo[playerid][pSpraycan]),
			PlayerInfo[targetid][pHunger]);
	        format(str6, sizeof(str),"{FF6600}Canh Cao Gangster: {FF9900}%s {99CCFF}/ Painball Tokens: %s / Keys: %d / Tan So Radio: %s khz", number_format(PlayerInfo[playerid][pGangWarn]),
			number_format(PlayerInfo[playerid][pPaintTokens]),
			PlayerInfo[playerid][pLock],
			PlayerInfo[playerid][pRadioFreq]);
	        format(str7, sizeof(str),"{CC9900}Truy Na: %s {99CCFF}/ Truy To: %s sao / Da bi bat: %s lan {99CCFF}/ {FF6600}Canh Cao: {FF9900}%s", PlayerInfo[playerid][pWantedLevel],
			number_format(PlayerInfo[playerid][pCrimes]),
			number_format(PlayerInfo[playerid][pArrested]),
			number_format(PlayerInfo[playerid][pWarns]));
	        format(str8, sizeof(str),"{FF9900}Cong Viec: {00CCCC}%s {99CCFF}( {66CC00}Level: {66CCFF}%d {99CCFF}) / {FF9900}Cong Viec 2: {00CCCC}%s {99CCFF}( {66CC00}Level: {66CCFF}%d {99CCFF}) / {66CC33}Thanh Pho: {FFCC00}%s",
			GetJobName(PlayerInfo[playerid][pJob]),
			GetJobLevel(playerid, PlayerInfo[playerid][pJob]),
			GetJobName(PlayerInfo[playerid][pJob2]),
			GetJobLevel(playerid, PlayerInfo[playerid][pJob2]),
			nation);
			format(str, sizeof(str), "%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s%s", str1,str2,str3,str4,str5,str6,str7,str8);
            ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, header, str, "OK", "");
		}
	}
	return 1;
}
...
stock DisplayOrders(playerid)
{
	...
	// xe truck // truck
		...
		TogglePlayerControllable(playerid, 1);
		DeletePVar(playerid, "IsFrozen"); */
		if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 482 || GetVehicleModel(GetPlayerVehicleID(playerid)) == 482 || IsABoat(GetPlayerVehicleID(playerid)))
		{
...
stock SpawnZombie(playerid)
{
	...
	TPPlayer(playerid, ZombieSpawns[tpto][0], ZombieSpawns[tpto][1], ZombieSpawns[tpto][2]);
...
public TeleportToShop(playerid)
{
	...
	{
		TPPlayer(playerid, 2957.9670, -1459.4045, 10.8092);
		SetPlayerInterior(playerid, 0);
		SetPlayerVirtualWorld(playerid, 0);
		TogglePlayerControllable(playerid, 1);
