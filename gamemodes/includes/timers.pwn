		...
		SetPlayerScore(i, PlayerInfo[i][pLevel]);
	   	...
	   	if(PlayerInfo[i][pBeingSentenced] > 1) {
			if(--PlayerInfo[i][pBeingSentenced] == 1)
			{
				...
				//Frozen[i] = 0;
				TPPlayer(i, 1415.5137,-1702.2272,13.5395);
...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		if(PlayerInfo[i][pConnectHours] < 0) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Vu khi se khong bi gioi han trong 0 gio choi dau tien khi tham gia may chu GOSAMP.TOP.");
		}
    	...
    	if(--EventKernel[EventTime] <= 0) {
    	    foreach(new i: Player)
			{
    			if( GetPVarInt( i, "EventToken" ) == 1 )
				{
					...
					TPPlayer(i,EventFloats[i][1],EventFloats[i][2],EventFloats[i][3]);
	...
	foreach(new i: Player)
	{
	    ...
	    if(GetPVarType(i, "Injured"))
	    {
	        ...
	        if(GetPVarInt(i, "EMSAttempt") != 0)
			{
				...
				{
					if(GetPlayerAnimationIndex(i) != 1508) ClearAnimations(i), ApplyAnimation(i, "SWAT", "GNSTWALL_INJURD", 4.1, 0, 1, 1, 1, 0, 1);
                 ...
                 /*SendClientMessageEx(i, COLOR_WHITE, "Ban da bi bat tinh, ban se duoc dua ve benh vien de dieu tri.");
                    ...
                    new string[200];
					GameTextForPlayer(i, "~r~Bi thuong~n~~w~/chapnhan chet hoac /hoisinhteam", 5000, 3);
                    format(string, sizeof(string), "(( Bam nut {FF0606}[ F ] {9ACD32}de hoi sinh nguoi nay.))");
                    SetPlayerChatBubble(i,string,COLOR_LIGHTGREEN,50.0,10000);
				}
				if(GetPVarInt(i, "EMSAttempt") == 1)
				{
					if(GetPlayerAnimationIndex(i) != 1508) ClearAnimations(i), ApplyAnimation(i, "SWAT", "GNSTWALL_INJURD", 4.1, 0, 1, 1, 1, 0, 1);
...
task ServerHeartbeat[1000]() {
	...
	foreach(new i: Player)
	{
		...
		if(playerTabbed[i] == 0) {
			if(PlayerInfo[i][pJailTime] > 0 && --PlayerInfo[i][pJailTime] <= 0) {
				if(strfind(PlayerInfo[i][pPrisonReason], "[IC]", true) != -1 || strfind(PlayerInfo[i][pPrisonReason], "[ISOLATE]", true) != -1) {
					...
					PlayerInfo[i][pVW] = 0;
					TPPlayer(i, -2028.5829,-96.5533,35.1641);
				}
				else {
				 	SetPlayerInterior(i, 0);
					PlayerInfo[i][pInt] = 0;
					SetPlayerVirtualWorld(i, 0);
					PlayerInfo[i][pVW] = 0;
	    			TPPlayer(i, 1544.5059,-1675.5673,13.5585);
				}
				...
				if(trigger)
				{
					...
					if(Lost == 1)
					{
						if(IsPlayerConnected(Boxer1) && IsPlayerConnected(Boxer2))
						{
							...
							{
								TPPlayer(Boxer1, 768.48, -73.66, 1000.57); TPPlayer(Boxer2, 768.48, -73.66, 1000.57);
							...
							TPPlayer(Boxer1, 765.8433,3.2924,1000.7186); TPPlayer(Boxer2, 765.8433,3.2924,1000.7186);
					...
					if (GetPVarInt(ii, "_BoxingQueue") == 1 && i != ii)
					{
						...
						if(biz == biz2)
						{
							if (Businesses[biz][bGymBoxingArena1][0] == INVALID_PLAYER_ID)
							{
								...
								TPPlayer(i, 2924.0735, -2293.3145, 8.0905);
								SetPlayerFacingAngle(i, 136.0062);
								TPPlayer(ii, 2920.4709, -2296.9460, 8.0905);
							...
							else if (Businesses[biz][bGymBoxingArena2][0] == INVALID_PLAYER_ID)
							{
								...
								TPPlayer(i, 2920.6958, -2257.4312, 8.0905);
								SetPlayerFacingAngle(i, 310.5444);
								TPPlayer(ii, 2924.3989, -2253.8279, 8.0905);
		...
		if (GetPVarInt(i, "_BoxingFightOver") != 0 && gettime() >= GetPVarInt(i, "_BoxingFightOver"))
		{
			...
			SetPlayerWeapons(i);
			TPPlayer(i, 2914.0706, -2263.0193, 7.2367);
		}
	}
