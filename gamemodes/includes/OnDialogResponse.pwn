...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == XEPHANG)
	{
	    if(response)
	    {
	        if(listitem == 0) //TOP 10 - KIM CUONG
	        {
                new Cache:Result, ss[2500];
				Result = mysql_query(MainPipeline, "SELECT `Credits`, `Username` FROM `accounts` ORDER BY `Credits` DESC LIMIT 10");
				if(cache_num_rows())
				{
				    new money, username[MAX_PLAYER_NAME + 1];
				    format(ss,sizeof(ss),"%sTOP\tNguoi choi\tCredits\n", ss);
    			...
    			cache_delete(Result);
	    		ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}TOP 10 - CREDITS", ss, "Dong y", "");
 			}
 			if(listitem == 1) //TOP 10 - MONEY
	        {
                new Cache:Result, ss[2500];
				Result = mysql_query(MainPipeline, "SELECT `Money`, `Username` FROM `accounts` ORDER BY `Money` DESC LIMIT 10");
				if(cache_num_rows())
				{
				    new money, username[MAX_PLAYER_NAME + 1];
    			...
    			cache_delete(Result);
	    		ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}TOP 10 - MONEY", ss, "Dong y", "");
 			}
 			if(listitem == 2) //TOP 10 - KILL
	        {
                new Cache:Result, ss[2500];
				Result = mysql_query(MainPipeline, "SELECT `ConnectedTime`, `Username` FROM `accounts` ORDER BY `ConnectedTime` DESC LIMIT 10");
				if(cache_num_rows())
				{
				    new money, username[MAX_PLAYER_NAME + 1];
				    format(ss,sizeof(ss),"%sTOP\tNguoi choi\tONLINE\n", ss);
					for(new i = 0; i < cache_num_rows(); ++i)
					{
						money = cache_get_row_int(i, 0);
						cache_get_row(i, 1, username);
    			...
    			cache_delete(Result);
	    		ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}TOP 10 - GIO CHOI", ss, "Dong y", "");
 			}
 			if(listitem == 3) //TOP 10 - VAT LIEU
	        {
                new Cache:Result, ss[2500];
				Result = mysql_query(MainPipeline, "SELECT `Materials`, `Username` FROM `accounts` ORDER BY `Materials` DESC LIMIT 10");
				if(cache_num_rows())
				{
				    new money, username[MAX_PLAYER_NAME + 1];
				    format(ss,sizeof(ss),"%sTOP\tNguoi choi\tVat Lieu\n", ss);
					for(new i = 0; i < cache_num_rows(); ++i)
					{
						money = cache_get_row_int(i, 0);
						cache_get_row(i, 1, username);
    			...
    			cache_delete(Result);
	    		ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}TOP 10 - VAT LIEU", ss, "Ok", "");
 			}
 			if(listitem == 4) //TOP 10 - LEVEL
	        {
                new Cache:Result, ss[2500];
				Result = mysql_query(MainPipeline, "SELECT `Level`, `Username` FROM `accounts` ORDER BY `Level` DESC LIMIT 10");
				if(cache_num_rows())
				{
				    new money, username[MAX_PLAYER_NAME + 1];
				    format(ss,sizeof(ss),"%sTOP\tNguoi choi\tCap Do\n", ss);
					for(new i = 0; i < cache_num_rows(); ++i)
					{
						money = cache_get_row_int(i, 0);
						cache_get_row(i, 1, username);
    			...
    			cache_delete(Result);
	    		ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}TOP 10 - LEVEL", ss, "Ok", "");
 			}
		}
	}
		  if(dialogid == NANGCAPXE && response)
      {
		    new ModelID = PlayerVehicleInfo[playerid][listitem][pvModelId];
		    if(ModelID == 601 || ModelID == 407 || ModelID == 490 || ModelID == 528  || ModelID == 470 || ModelID == 432 || ModelID == 425 || ModelID == 427 || ModelID == 433 || ModelID == 597 || ModelID == 596 || ModelID == 528 || ModelID == 428)
				return SendClientMessageEx(playerid, COLOR_GREY, "Xe nay khong thuoc dang duoc nang cap!");
		    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 0)
		    {
			    if(PlayerInfo[playerid][pCredits] >= 50000)
			    {
					if(PlayerVehicleInfo[playerid][listitem][pvNangcap] < 1)
					{
					    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 0)
					    {
							PlayerVehicleInfo[playerid][listitem][pvNangcap] = 1;
							PlayerInfo[playerid][pCredits] -= 50000;
							SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da nang cap phuong tien thanh cong, (-50000 Credits)");
						}
						else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien cua ban da dat level 1 , khong the nang cap tiep");
					}
					else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien ban da vuot qua level 1 , khong the nang cap tiep");
				}
				else SendClientMessageEx(playerid, COLOR_RED, "Ban khong du Coin (50000 Credits)");
			}
			else if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 1)
			{
			    if(PlayerInfo[playerid][pCredits] >= 60000)
			    {
					if(PlayerVehicleInfo[playerid][listitem][pvNangcap] < 2)
					{
					    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 1)
					    {
							PlayerVehicleInfo[playerid][listitem][pvNangcap] = 2;
							PlayerInfo[playerid][pCredits] -= 60000;
							SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da nang cap phuong tien thanh cong, (-60000 Credits)");
						}
						else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien cua ban da dat level 2 , khong the nang cap tiep");
					}
					else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien ban da vuot qua level 2 , khong the nang cap tiep");
				}
				else SendClientMessageEx(playerid, COLOR_RED, "Ban khong du Coin (60000 Credits)");
			}
			else if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 2)
			{
			    if(PlayerInfo[playerid][pCredits] >= 70000)
			    {
					if(PlayerVehicleInfo[playerid][listitem][pvNangcap] < 3)
					{
					    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 2)
					    {
							PlayerVehicleInfo[playerid][listitem][pvNangcap] = 3;
							PlayerInfo[playerid][pCredits] -= 70000;
							SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da nang cap phuong tien thanh cong, (-70000 Credits)");
						}
						else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien cua ban da dat level 3 , khong the nang cap tiep");
					}
					else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien ban da vuot qua level 3 , khong the nang cap tiep");
				}
				else SendClientMessageEx(playerid, COLOR_RED, "Ban khong du Coin(15000 Credits)");
			}
			else if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 3)
			{
			    if(PlayerInfo[playerid][pCredits] >= 80000)
			    {
					if(PlayerVehicleInfo[playerid][listitem][pvNangcap] < 4)
					{
					    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 3)
					    {
							PlayerVehicleInfo[playerid][listitem][pvNangcap] = 4;
							PlayerInfo[playerid][pCredits] -= 80000;
							SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da nang cap phuong tien thanh cong, (-80000 Credits)");
						}
						else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien cua ban da dat level 4 , khong the nang cap tiep");
					}
					else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien ban da vuot qua level 4 , khong the nang cap tiep");
				}
				else SendClientMessageEx(playerid, COLOR_RED, "Ban khong du Coin (80000 Credits)");
			}
			else if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 4)
			{
			    if(PlayerInfo[playerid][pCredits] >= 90000)
			    {
					if(PlayerVehicleInfo[playerid][listitem][pvNangcap] < 5)
					{
					    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 4)
					    {
							PlayerVehicleInfo[playerid][listitem][pvNangcap] = 5;
							PlayerInfo[playerid][pCredits] -= 90000;
							SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da nang cap phuong tien thanh cong, (-90000 Credits)");
						}
						else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien cua ban da dat level 5 , khong the nang cap tiep");
					}
					else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien ban da vuot qua level 5 , khong the nang cap tiep");
				}
				else SendClientMessageEx(playerid, COLOR_RED, "Ban khong du Coin (90000 Credits)");
			}
			else if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 5)
			{
			    if(PlayerInfo[playerid][pCredits] > 100000)
			    {
					if(PlayerVehicleInfo[playerid][listitem][pvNangcap] < 6)
					{
					    if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 5)
					    {
							PlayerVehicleInfo[playerid][listitem][pvNangcap] = 6;
							PlayerInfo[playerid][pCredits] -= 100000;
							SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da nang cap phuong tien thanh cong, (-100000 Coin)");
						}
						else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien cua ban da dat level 6 , khong the nang cap tiep");
					}
					else SendClientMessageEx(playerid, COLOR_RED, "Phuong tien ban da dat level 6 , khong the nang cap tiep");
				}
				else SendClientMessageEx(playerid, COLOR_RED, "Ban khong du Coin (100000 Coin)");
			}
			else if(PlayerVehicleInfo[playerid][listitem][pvNangcap] == 6) return  SendClientMessageEx(playerid, COLOR_RED, "Xe cua ban hien tai dang o cap do cao nhat [Cap do 6]");
	  }
    ...
    if(dialogid == DIALOG_NGHENHACNE)
    {
    if(listitem == 0) //cuoi thoi
	{
		PlayAudioStreamForPlayerEx(playerid, "https://c1-ex-swe.nixcdn.com/NhacCuaTui1021/CuoiThoi-MasewMasiuBRayTAPVietNam-7085648.mp3");
    }
	if(listitem == 1) //Do Toc 2
	{
		PlayAudioStreamForPlayerEx(playerid, "https://c1-ex-swe.nixcdn.com/NhacCuaTui1020/DoToc2-MasewDoMixiPhucDuPhao-7064730.mp3");
	}
    if(listitem == 2) // 3 1 0 7 3
	{
		PlayAudioStreamForPlayerEx(playerid, "https://c1-ex-swe.nixcdn.com/Unv_Audio199/31073-WNDuonggNautitie-7059323.mp3");
	}
    if(listitem == 3) // Phai Chang Em Da Yeu
	{
		PlayAudioStreamForPlayerEx(playerid, "https://c1-ex-swe.nixcdn.com/NhacCuaTui1011/PhaiChangEmDaYeu-JukySanRedT-6940932.mp3");
	}
    if(listitem == 4) // Nang Tho
	{
		PlayAudioStreamForPlayerEx(playerid, "https://c1-ex-swe.nixcdn.com/NhacCuaTui1001/NangTho-HoangDung-6413381.mp3");
	}
    if(listitem == 5) // Ai no
	{
		PlayAudioStreamForPlayerEx(playerid, "https://c1-ex-swe.nixcdn.com/NhacCuaTui1021/AiNo1-MasewKhoiVu-7078913.mp3");
	}
	if(listitem == 6) // SCTM
	{
	    PlayAudioStreamForPlayerEx(playerid, "https://seaf20.ytop1.com/files/2020/6/25/song_cho_het_doi_thanh_xuan_music_video_dick_x_xam_x_tuyet_ZgpF63IByoK-gucZaB0y.mp3");
	}
	if(listitem == 7)
	{
		SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "{AA3333}[GOSAMP]{FFFF00}He thong nghe nhac");
	}
 }
    ...
    if(dialogid == AUDIO_URL_BOOM)
	{
	    if(response)
	    {
	        ...
	        if(listitem == 1)
	        {
		 	 	ShowPlayerDialog(playerid, AUDIO_URL_LIST, DIALOG_STYLE_LIST, "Lua chon Playlist", "Anh nhan ra\nBa thang ban\nTra gia\nNguoi ngoai cuoc\nDem tan\nHeat Attack\nNuma Numa\nLen la len", "OK", "Tu choi");
		    }
	    }
	}
	...
	if(dialogid == AUDIO_URL_LIST)
	{
	    ...
	    {
		 	//Bai hat 1
		 	SendAudioURLToRange("http://tmp.mp3.stream.nixcdn.com/a2fd4d76f886307c167e10cfe535a898/519aa3bc/NhacCuaTui215/AnhNhanRa-KhacViet_3xkjt.mp3",GetPVarFloat(playerid, "pBoomBoxX"),GetPVarFloat(playerid, "pBoomBoxY"),GetPVarFloat(playerid, "pBoomBoxZ"),50);
	        SendClientMessage(playerid, COLOR_YELLOW, "Ban da chon bai hat Anh nhan ra");
	    }
	    if(listitem == 2)
	    {
		 	//Bai hat 2
	...
	if(dialogid == TIMVIECLAM)
    {
        if(response)
        {
            if(listitem == 0)// Tham tu (LS)
            {
                SetPlayerCheckpoint(playerid, 1380.7683,-1088.7391,27.3844, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 1)// Buon vu khi (LS)
            {
                SetPlayerCheckpoint(playerid, 1366.4325,-1275.2096,13.5469, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 2)// Buon thuoc phien (LS)
            {
                SetPlayerCheckpoint(playerid, 2166.3772,-1675.3829,15.0859, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 3)// Ve si (LS)
            {
                SetPlayerCheckpoint(playerid, 2226.1716,-1718.1792,13.5165, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 4)// Luat su (LS)
            {
                SetPlayerCheckpoint(playerid, 1555.1298,-1675.5709,16.1953, 5);
                SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "Ban hay tim den chu (i) ben trong HQ LSPD roi /xinviec!");
                CP[playerid] = 252007;
            }
            else if(listitem == 5)// Tai xe Taxi (LS)
            {
                SetPlayerCheckpoint(playerid, 1808.4767,-1885.1515,13.5851, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 6)// Tho sua xe (LS)
            {
                SetPlayerCheckpoint(playerid, 1911.7753,-1872.7100,13.5475, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 7)// Gai diem (LS)
            {
                SetPlayerCheckpoint(playerid, 1878.0184,-1627.1929,13.3620, 5);
                SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "Ban hay tim den chu (i) ben trong Bar roi /xinviec!");
                CP[playerid] = 252007;
            }
            
            else if(listitem == 8)// Boxer
            {
                SetPlayerCheckpoint(playerid, 2292.2490,-1722.9474,13.5469, 5);
                SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "Ban hay tim den chu (i) ben trong Phong tap vo roi /xinviec!");
                CP[playerid] = 252007;
            }
            else if(listitem == 9)// Buon ma tuy (LS)
            {
                SetPlayerCheckpoint(playerid, 2354.2808,-1169.2959,28.0066, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 10)// Tho thu cong (LS)
            {
                SetPlayerCheckpoint(playerid, 2195.8335,-1973.0638,13.5589, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 11)// Nguoi pha che (LS)
            {
                SetPlayerCheckpoint(playerid, 1836.9965,-1682.4426,13.3246, 5);
                SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "Ban hay tim den chu (i) ben trong ALHAMBRA roi /xinviec!");
                CP[playerid] = 252007;
            }
            else if(listitem == 12)// Trucker (LS)
            {
                SetPlayerCheckpoint(playerid, 2450.2000,-2126.0259,13.5469, 5);
                CP[playerid] = 252007;
            }
            else if(listitem == 13)// Pizza boy (LS)
            {
                SetPlayerCheckpoint(playerid, 2103.9937,-1809.6488,13.5547, 5);
                CP[playerid] = 252007;
            }
        }
    }
	...
	if(dialogid == TIMDUONG)
    {
        if(response)
        {
            if(listitem == 0)// Shop xe (LS)
            {
                SetPlayerCheckpoint(playerid, 1638.0868,-1896.2919,13.5544, 5);
                CP[playerid] = 252006;
            }
            if(listitem == 1)// Tru so canh sat (LS)
            {
                SetPlayerCheckpoint(playerid, 1543.4709,-1675.2252,13.5565, 5);
                CP[playerid] = 252006;
            }
            if(listitem == 2)// VIP (LS)
            {
                SetPlayerCheckpoint(playerid, 1806.9241,-1579.1755,13.478, 5);
                CP[playerid] = 252006;
            }
            if(listitem == 3)// Benh vien (LS)
            {
                SetPlayerCheckpoint(playerid, 2030.9778,-1414.4093,16.9922, 5);
                CP[playerid] = 252006;
            }
            if(listitem == 4)// Tram xang (LS)
            {
                SetPlayerCheckpoint(playerid, 1935.5212,-1771.0665,13.3828, 5);
                CP[playerid] = 252006;
            }
            if(listitem == 5)// Cua hang tap hoa 24/7 (LS)
            {
                SetPlayerCheckpoint(playerid, 2421.4641,-1740.1552,13.5469, 5);
                CP[playerid] = 252006;
            }
        }
    }
		...
		else if(dialogid == REGISTERREF)
		{
		    if(response)
		    {
                ...
                format(szQuery, sizeof(szQuery), "SELECT `Username` FROM `accounts` WHERE `Username` = '%s'", szEscape);
         		...
         		PlayerInfo[playerid][pGVIPVoucher] += 30;
				PlayerInfo[playerid][pCarVoucher] += 30;
				PlayerInfo[playerid][pCredits] += 100;
		  		PlayerInfo[playerid][pCash] += 1000000;
		  		PlayerInfo[playerid][pVehVoucher] = 100;
		  		PlayerInfo[playerid][pConnectHours] = 24;
				SendClientMessage(playerid, COLOR_YELLOW, "Ban nhan duoc 30 cai VIP 30 ngay, 100 Credit va 1,000,000$ va 100 car xe va 24 gio va 30 phieu xe faction");
				...
				TPPlayer(playerid, -2414.6541,521.6945,29.9297);
			...
			//	TPPlayer(playerid, 1607.0160,-1510.8218,-10.0);
			...
			//	SetPlayerCameraLookAt(playerid, 1607.0160,-1510.8218,207.4438);
				...
				TPPlayer(playerid,1716.1129,-1880.0715,-10.0);
    ...
    if(dialogid == RCPEDITMENU2)
    {
        if (response)
	    {
			...
			else if(listitem == 3) // view checkpoint
			{
				...
				TPPlayer(playerid, EventRCPX[ConfigEventCPId[playerid]], EventRCPY[ConfigEventCPId[playerid]], EventRCPZ[ConfigEventCPId[playerid]]);
	        ...
	        switch(listitem)
	        {
	            case 0: // Jailtime
				{
				    if(PlayerInfo[playerid][pNMuteTotal] < 4)
				    {
						...
						TPPlayer(playerid, OOCPrisonSpawns[rand][0], OOCPrisonSpawns[rand][1], OOCPrisonSpawns[rand][2]);
	    ...
	    if(response == 1)
	    {
	        for(new i = 0; i < MAX_ARENAS; i++)
	        {
	            if(listitem == i)
	            {
					...
					TPPlayer(playerid, PaintBallArena[i][pbDeathmatch1][0],PaintBallArena[i][pbDeathmatch1][1],PaintBallArena[i][pbDeathmatch1][2]);
	...
	if(dialogid == PBEDITARENAMENU)
	{
	    ...
	    else
	    {
	        ...
	        {
	            TPPlayer(playerid, GetPVarFloat(playerid, "pbOldX"),GetPVarFloat(playerid, "pbOldY"),GetPVarFloat(playerid, "pbOldZ"));
		...
		if(response == 1)
		{
		    ...
		    switch(listitem)
		    {
		        case 0: // Spawn Positions 1
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbDeathmatch1][0],PaintBallArena[arenaid][pbDeathmatch1][1],PaintBallArena[arenaid][pbDeathmatch1][2]);
		        ...
		        case 1: // Spawn Positions 2
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbDeathmatch2][0],PaintBallArena[arenaid][pbDeathmatch2][1],PaintBallArena[arenaid][pbDeathmatch2][2]);
		        ...
		        case 2: // Spawn Positions 3
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbDeathmatch3][0],PaintBallArena[arenaid][pbDeathmatch3][1],PaintBallArena[arenaid][pbDeathmatch3][2]);
		        ...
		        case 3: // Spawn Positions 4
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbDeathmatch4][0],PaintBallArena[arenaid][pbDeathmatch4][1],PaintBallArena[arenaid][pbDeathmatch4][2]);
		...
		if(response == 1)
		{
		    ...
		    switch(listitem)
		    {
		        case 0: // Red Spawn Positions 1
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbTeamRed1][0],PaintBallArena[arenaid][pbTeamRed1][1],PaintBallArena[arenaid][pbTeamRed1][2]);
				...
				case 1: // Red Spawn Positions 2
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbTeamRed2][0],PaintBallArena[arenaid][pbTeamRed2][1],PaintBallArena[arenaid][pbTeamRed2][2]);
				...
				case 2: // Red Spawn Positions 3
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbTeamRed3][0],PaintBallArena[arenaid][pbTeamRed3][1],PaintBallArena[arenaid][pbTeamRed3][2]);
				...
				case 3: // Blue Spawn Positions 1
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbTeamBlue1][0],PaintBallArena[arenaid][pbTeamBlue1][1],PaintBallArena[arenaid][pbTeamBlue1][2]);
				...
				case 4: // Blue Spawn Positions 2
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbTeamBlue2][0],PaintBallArena[arenaid][pbTeamBlue2][1],PaintBallArena[arenaid][pbTeamBlue2][2]);
				...
				case 5: // Blue Spawn Positions 3
		        {
		            ...
		            TPPlayer(playerid, PaintBallArena[arenaid][pbTeamBlue3][0],PaintBallArena[arenaid][pbTeamBlue3][1],PaintBallArena[arenaid][pbTeamBlue3][2]);
	    ...
	    if(response == 1)
	    {
	        ...
	        switch(listitem)
	        {
	            case 0: // Red Flag
	            {
					...
					TPPlayer(playerid,PaintBallArena[arenaid][pbFlagRedSpawn][0],PaintBallArena[arenaid][pbFlagRedSpawn][1],PaintBallArena[arenaid][pbFlagRedSpawn][2]);
					SetPlayerInterior(playerid, PaintBallArena[arenaid][pbInterior]);
					SetPlayerVirtualWorld(playerid, PaintBallArena[arenaid][pbVirtual]);
	            }
	            case 1: // Blue Flag
	            {
					...
					TPPlayer(playerid,PaintBallArena[arenaid][pbFlagBlueSpawn][0],PaintBallArena[arenaid][pbFlagBlueSpawn][1],PaintBallArena[arenaid][pbFlagBlueSpawn][2]);
					SetPlayerInterior(playerid, PaintBallArena[arenaid][pbInterior]);
					SetPlayerVirtualWorld(playerid, PaintBallArena[arenaid][pbVirtual]);
				}
	        }
	...
	if (dialogid == ELEVATOR3 && response)
    {
		if (listitem == 0)
		{
			TPPlayer(playerid, 1564.8, -1666.2, 28.3);
			SetPlayerInterior(playerid, 0);
		...
		{
			TPPlayer(playerid, 1568.6676, -1689.9708, 6.2188);
		 	SetPlayerInterior(playerid, 0);
		 	PlayerInfo[playerid][pVW] = 0;
			SetPlayerVirtualWorld(playerid, 0);
		}
	...
	if (dialogid == ELEVATOR && response)
    {
		if (listitem == 0)
		{
			TPPlayer(playerid, 276.0980, 122.1232, 1004.6172);
							...
							if(GetPlayerCash(playerid) < 1000)
								{
									...
									TPPlayer(playerid, slx, sly, slz+1.2);
								    TogglePlayerControllable(playerid,1);
								}
							else
								{
	...
	if(dialogid == 1348)
	{
	    if(response)
	    {
	        ...
	        TPPlayer(playerid, carPosF[0], carPosF[1], carPosF[2]);
		}
	}
	        ...
	        for(new i = 0; i < MAX_PLAYERVEHICLES; i++)
	        {
	            if(listitem == i)
	            {
					...
					if(PlayerVehicleInfo[id][i][pvId] > INVALID_PLAYER_VEHICLE_ID)
					{
						...
						SetPlayerInterior(playerid,0);
						TPPlayer(playerid, carPos[0], carPos[1], carPos[2]);
					}
					else
					{
	...
	if(dialogid == VEHICLESTORAGE && response) {
		...
		else if((PlayerInfo[playerid][pRVehRestricted] > gettime() || PlayerVehicleInfo[playerid][listitem][pvRestricted] > gettime()) && IsRestrictedVehicle(PlayerVehicleInfo[playerid][listitem][pvModelId]))
        {
			...
			if(PlayerInfo[playerid][pDonateRank] == 0 && VehicleSpawned[playerid] >= 2) {
                ...
                new iVeh = CreateVehicle(PlayerVehicleInfo[playerid][listitem][pvModelId], X+2,Y,Z, PlayerVehicleInfo[playerid][listitem][pvPosAngle],PlayerVehicleInfo[playerid][listitem][pvColor1], PlayerVehicleInfo[playerid][listitem][pvColor2], -1);
	...
	if(dialogid == VIPWEPSMENU)
	{
	    ...
	    switch( listitem )
	    {
	        case 0:
	        {
				...
				SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} Deagle");
	        }
	        case 1:
	        {
				GivePlayerValidWeapon(playerid, 25, 60000);
				SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} ShotGun");
	        }
	        case 2:
	        {
				GivePlayerValidWeapon(playerid, 29, 60000);
				SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} MP5");
	        }
	        case 3:
	        {
                GivePlayerValidWeapon(playerid, 31, 60000);
                SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} M4");
	        }
	        case 4:
	        {
                GivePlayerValidWeapon(playerid, 34, 60000);
                SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} Sniper");
	        }
	        case 5:
	        {
                GivePlayerValidWeapon(playerid, 27, 60000);
                SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} Spas12");
	        }
	        case 6:
	        {
                GivePlayerValidWeapon(playerid, 28, 60000);
                SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} Micro Uzi");
	        }
	        case 7:
	        {
                GivePlayerValidWeapon(playerid, 33, 60000);
                SendClientMessageEx(playerid, COLOR_GRAD2, "{ffffff}Ban Da Lay Thanh Cong Vu Khi {00ff00} Rifle");
	    	}
		}
	}
	if( dialogid == 3497) //famed change skin
	{
		...
		if(response)
		{
			...
			{
				SendClientMessageEx(playerid, COLOR_GREY, "ID Skin khong hop le, Skin ID phai tu 1-311 !");
	...
	if(dialogid == 7483) // VIP Locker /viplocker
	{
		if(response)
		{
			if(listitem == 0)
			{
			    ...
			    SetPlayerHealth(playerid, 100);
				SendClientMessageEx(playerid, COLOR_YELLOW, "GOSAMP: Ban da su dung mot tui cuu thuong, bay gio ban co 100.0 HP.");
			}
			if(listitem == 1)
			{
				new Float:armour;
				...
				if(PlayerInfo[playerid][pDonateRank] == 0)
				{
					...
					SetPlayerArmor(playerid, 100);
					SendClientMessageEx(playerid, COLOR_YELLOW, "GOSAMP: Ban Vua Su Dung ! Kevlar Vest.");
				}
			...
			if(listitem == 3)
			{
			    ...
			    {
			    	SendClientMessageEx(playerid, COLOR_YELLOW, "GOSAMP: Ban phai co Silver VIP tro len moi co the truy cap vao tu do quan ao . Ban co the su dung bat ki skin nao");
			    }
			}
			if(listitem == 4)
			{
  				if(PlayerInfo[playerid][pDonateRank] >= 0)
            	{
			    	ShowPlayerDialog(playerid, 7484, DIALOG_STYLE_LIST, "VIP: Nghe nghiep", "Tham tu\nLuat su\nGai diem\nBan thuoc vien\nVe si\nTho sua xe\nBan vu khi\nVo si\nBuon ma tuy\nLai Taxi\nTho thu cong\nNguoi pha che\nNguoi cho hang\nGiao Pizza", "Tien hanh", "Huy bo");
		    	}
			    else
			    {
			    	SendClientMessageEx(playerid, COLOR_YELLOW, "GOSAMP: Ban phai co Silver VIP tro len moi co the truy cap vao danh sach nhan viec lam.");
			    }
			}
			if(listitem == 5)
			{
			    ShowPlayerDialog(playerid, 7486, DIALOG_STYLE_LIST, "GOSAMP: Mau nick VIP", "Bat\nTat", "Chon", "Huy bo");
			}
		}
	}
...
/*	if(dialogid == MDC_CHECK && response)
		...
		if(response)
		{
			...
			switch(arresttype)
			{
				case 0, 1: { //arrest
					...
					TPPlayer(suspect, LSPDJailSpawns[rand][0], LSPDJailSpawns[rand][1], LSPDJailSpawns[rand][2]);
				...
				case 2: { // /docarrest
					...
					TPPlayer(suspect, DocPrison[rand][0], DocPrison[rand][1], DocPrison[rand][2]);
					if(PlayerInfo[suspect][pDonateRank] >= 2)
					{
						PlayerInfo[suspect][pJailTime] = ((time*60)*75)/100;
					}
	...
	else if(dialogid == DIALOG_CDBUY)
	{
		...
		if(response)
		{
		    ...
		    TPPlayer(playerid, Businesses[d][bParkPosX][v], Businesses[d][bParkPosY][v], Businesses[d][bParkPosZ][v]+2);
		    TogglePlayerControllable(playerid, 1);
		    new cost;
		    if(PlayerInfo[playerid][pDonateRank] < 1)
            {
