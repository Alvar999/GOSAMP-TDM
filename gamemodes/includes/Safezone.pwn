 	    ...
 	    if(zombieevent == 0)
		{
	  		if(IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
	  		{
				if(EnterSafeZones[playerid] < 0)
	            {
	                ...
	                SendClientMessageEx(playerid, COLOR_WHITE, "{00ff00}Ban da vao ben trong khu vuc an toan.");
	                //GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~g~SAFEZONE", 2000, 1);
	                break;
	            }
	  		...
	  		if(Sz == EnterSafeZones[playerid] && EnterSafeZones[playerid] >= 0)
	        {
	            if(!IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
	            {
	                ...
	                SendClientMessageEx(playerid, COLOR_WHITE, "{ffff00}Ban da ra khoi khuc vuc an toan.");
	                //GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~r~SAFEZONE", 2000, 1);
	            }
	            break;
	        }
        }
