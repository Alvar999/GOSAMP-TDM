...
public OnPlayerUpdate(playerid)
{
	...
	format(string, sizeof(string), "%d/%d", PlayerInfo[playerid][pExp]);
	PlayerTextDrawSetString(playerid, kontenTD[playerid][8], string);
	new kimcuongtext[42];
format(kimcuongtext,sizeof(kimcuongtext),"~b~Kim Cuong: ~w~%s", number_format(PlayerInfo[playerid][pCredits]));
PlayerTextDrawSetString(playerid,KimCuongShow[playerid],kimcuongtext);
	...
	// Anti Hack Minigun
	if(GetPlayerWeapon(playerid) == 38 && PlayerInfo[playerid][pAdmin] ==
0) {
	   Kick(playerid);
   }
	//Khu an toan
	for(new Sz; Sz < MAX_SZ; Sz++)
	{
		...
		{
			if(PlayerInfo[playerid][pMember] == -1) return SetPlayerArmedWeapon(playerid, 0);
		}
	}
	return 1;
}
...
public OnPlayerClickMap(playerid, Float:fX, Float:fY, Float:fZ)
{
    ...
    SetPVarInt(playerid, "ClickMapa", gettime() + 10);
    if(PlayerInfo[playerid][pAdmin] >= 2)
	{
	    new zone[MAX_ZONE_NAME];
	    Get3DZone(fX,fY,fZ, zone, sizeof(zone));
		...
		if(GetPlayerState(playerid) == 2)
		{
			...
			new string[1280];
			format(string, sizeof(string), "Ban da duoc dich chuyen den {00ffff}%s", zone);
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid)
{
	...
	if(InfoMessage[playerid] == true) // version 2
	{
       if(issuerid != INVALID_PLAYER_ID)
       {
        new
        ...
        format(infoString, sizeof(infoString), "[Chien Dau] Ban bi {00bf00}%s{ffffff} su dung {ffff00}%s{ffffff} gay ton that {ff0000}%.02f", attackerName, weaponName, amount);
...
hook OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid, bodypart)
{
    ...
    if(GetPVarInt(playerid, "IsInArena") == -1 && GetPVarInt(damagedid, "IsInArena") == -1 && GetPVarInt( playerid, "EventToken" ) == 0) {
	    if(!(IsPlayerInRangeOfPoint(playerid, 1.0, pos[0], pos[1], pos[2]))) {
        	if(GetPVarInt(damagedid, "Injured") == 0 && GetPVarType(damagedid, "pGodMode") == 0) {
	            if(weaponid == 22) {
	                amount = 5;
				}
				else if(weaponid == 23) {
			...
			if(GetPVarType(damagedid, "pGodMode") == 1) amount = 0;
			if(GetPVarInt(damagedid, "Injured") == 1) amount = 0;
			if(PlayerInfo[damagedid][pJailTime] > 0) amount = 0;
	...
	//
    if(PlayerInfo[playerid][pLevel] == 1 && PlayerInfo[playerid][pMember] == INVALID_GROUP_ID && PlayerInfo[playerid][pConnectHours] < 2 && PlayerInfo[playerid][pAdmin] == 0) {
    ...
    if(GetPVarInt(playerid, "IsInArena") == -1 && GetPVarInt( playerid, "EventToken" ) == 0)
	{
	    ...
	    {
	        AntiKillAll[playerid]++;
	        if(AntiKillAll[playerid] == 10)
	        {
	            AntiKillAll[playerid] = 0;
    ...
    for(new Sz; Sz < MAX_SZ; Sz++)
    {
		...
		if(IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
		{
		    if(weaponid != 0)
	        {
				...
				if(AntiSafeZone[playerid] == 3)
				{
				    ...
				    ///new safe[1280];
				    //format(safe, sizeof(safe), "Nguoi choi (( %s )) da bi kick do no sung trong Safezone.", attackercc);
				    //SendClientMessageToAllEx(COLOR_LIGHTRED, safe);
				    new safe[1280];
  		...
  		if(!IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
		{
  			if(IsPlayerInRangeOfPoint(damagedid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
  			{
  			    ...
  			    {
	       			GameTextForPlayer(playerid, "~r~SAFEZONE - GTAVNS", 2000, 3);
			       	TogglePlayerControllable(playerid, 0);
			       	SetTimerEx("LoadPlayerZone", 5000, 0, "d", playerid);
				    GetPlayerHealth(playerid,php);
    ...
    new Float:Distance = GetDistanceBetweenPlayers(damagedid, playerid);
    if(Distance > 350) {
	    new string[128];
	    format(string, sizeof(string), "Laur.: %s da bi kick boi Laur., ly do: Hack Fire Xa", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
	    SendClientMessageToAll(COLOR_LIGHTRED, string);
	    KickWithMessage(playerid, "Ban da bi kick, Ly do: Hack Fire Xa");
        Kick(playerid);
        return 1;
}
    if(amount > 70) {
    new string[128];
    format(string, sizeof(string), "Laur.: %s da bi kick boi Laur., ly do: Hack Dame", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
    SendClientMessageToAll(COLOR_LIGHTRED, string);
    KickWithMessage(playerid, "Ban da bi kick, Ly do: Hack Dame");
	Kick(playerid);
	return 1;
}
	if (damagedid == INVALID_PLAYER_ID) return 1;
    ...
    GetWeaponName(weaponid, weaponname, sizeof (weaponname));
    format(string, sizeof(string), "[Chien Dau] Ban da dung {00bf00}%s{ffffff} gay cho {00bf00}%s{ffffff} ton that {ff0000}%.02f", weaponname, victim, amount);
	...
	for(new Sz; Sz < MAX_SZ; Sz++)
	{
		...
		{// If Player In Safe Zone
			...
			{
				GameTextForPlayer(playerid, "~y~Ban dang o trong khu an toan", 1000, 3);
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    ...
    if(GetPVarType(playerid, "Pizza") && !(IsAPizzaCar(vehicleid)))
	{
		...
		TPPlayer(playerid, slx, sly, slz+1.3);
	...
	if(!ispassenger)
	{
	    ...
	    {
	        if(PlayerInfo[playerid][pLevel] < 99999)
	        {
				if(gettime() > GetPVarInt(playerid, "timeWepVeh"))
				{
					new szString[128];
					format(szString, sizeof(szString), "{AA3333}Laur.{FFFF00}: %s (ID: %d) Nguoi Choi Buoc Len Mot Chiec Xe Hang Nang Faction (Vehicle ID: %d) (Level: %d)", GetPlayerNameEx(playerid), playerid, vehicleid, PlayerInfo[playerid][pLevel]);
...
{
    if(pickupid == DeathBox) {
		if(GetPVarInt(playerid, "injured") < 1) { //khi tr?ng thái knock ko nh?n dc
			SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan duoc 1500 Credits Tu Hop Qua Death Box !");
			PlayerInfo[playerid][pCredits] += 1500;
			DestroyPickup(DeathBox);
		}
	}
    return 1;
}
...
public OnPlayerConnect(playerid)
{
    ...
    format( strconnect, sizeof( strconnect ), "{ff5f00}[Login]{5fffff} %s (ID: %d) (IP: %s)- (Email: %s) - (Level : %d) , Dang Tham Gia May Chu Laur...", name, playerid, playerip, PlayerInfo[playerid][pEmail], PlayerInfo[playerid][pLevel]);
	foreach(new i: Player)
    {
        if(PlayerInfo[i][pAdmin] >= 2)
        {
            SendClientMessageEx(i,COLOR_LIGHTBLUE, strconnect);
        }
    }
    ...
    FlagTele[playerid] = 0;
    DeletePVar(playerid, "ClickMapa");
    DeletePVar(playerid, "PX");
    DeletePVar(playerid, "PY");
    DeletePVar(playerid, "PZ");
    DeletePVar(playerid, "PXt");
    DeletePVar(playerid, "PYt");
    DeletePVar(playerid, "PZt");
	...
	SendClientMessage( playerid, COLOR_WHITE, "SERVER: Chao mung ban den voi Server GOSAMP.TOP" );
	SendClientMessage( playerid, COLOR_WHITE, "Laur." );
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    DeletePVar(playerid,"de");
    DeletePVar(playerid,"de1");
    DeletePVar(playerid,"spas");
    DeletePVar(playerid,"spas1");
    DeletePVar(playerid,"sni");
    DeletePVar(playerid,"sni1");
    DeletePVar(playerid,"ril");
			...
			if(Boxer1 == playerid)
			{
				if(IsPlayerConnected(Boxer2))
				{
					if(IsPlayerInRangeOfPoint(PlayerBoxing[Boxer2], 20.0, 768.94, -70.87, 1001.56))
					{
						...
						TPPlayer(Boxer2, 768.48, -73.66, 1000.57);
						SetPlayerInterior(Boxer2, 7);
						GameTextForPlayer(Boxer2, "~r~Match interupted", 5000, 1);
					}
					PlayerBoxing[Boxer2] = 0;
					TPPlayer(Boxer2, 765.8433,3.2924,1000.7186);
			...
			else if(Boxer2 == playerid)
			{
				if(IsPlayerConnected(Boxer1))
				{
					if(IsPlayerInRangeOfPoint(PlayerBoxing[Boxer1],20.0,764.35, -66.48, 1001.56))
					{
						...
						TPPlayer(Boxer1, 768.48, -73.66, 1000.57);
						SetPlayerInterior(Boxer1, 7);
						GameTextForPlayer(Boxer1, "~r~Match interupted", 5000, 1);
					}
					PlayerBoxing[Boxer1] = 0;
					TPPlayer(Boxer1, 765.8433,3.2924,1000.7186);
...
public OnPlayerDeath(playerid, killerid, reason)
{
    for(new Sz; Sz < MAX_SZ; Sz++)
    {
        if(zombieevent == 0)
		{
						...
						case 50:{
							if(IsAHelicopter(GetPlayerVehicleID(killerid)))
							{
							    ...
							    if(SoLanBK[killerid] == 2)
							    {
									...
									new rand = random(sizeof(OOCPrisonSpawns));
									TPPlayer(killerid, OOCPrisonSpawns[rand][0], OOCPrisonSpawns[rand][1], OOCPrisonSpawns[rand][2]);
	        ...
	        if(GetPVarInt(playerid, "IsInArena") < 1) { //trong arena ko roi box
		    new Float:x, Float:y, Float:z;
		    GetPlayerPos(playerid, x, y, z);
		    DeathBox = CreatePickup(19055, 1, x, y, z);
		    SetTimerEx("BoxTimer", 30000, 0, "i", playerid); //time box bi?n m?t
	    }
	new string[128];
	format(string, sizeof(string), "{AA3333}[*K]{FFFF00}: %s (ID :%d) da ha guc %s (ID :%d).Ly do : {40FFFF}%s .", GetPlayerNameEx(killerid), killerid, GetPlayerNameEx(playerid), playerid,GetDeath(reason));
        ProxDetector(20.0, playerid, string,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE, 1);
        format(string, sizeof(string), "{AA3333}Chien Dau{FFFF00}: %s (ID :%d) Da Ha Guc Ban .", GetPlayerNameEx(killerid), killerid);
        SendClientMessageEx(playerid, COLOR_YELLOW, string);
        format(string, sizeof(string), "{AA3333}Chien Dau{FFFF00}: Ban da ha guc %s (ID %d).", GetPlayerNameEx(playerid), playerid);
	...
	if(IsPlayerConnected(playerid) && IsPlayerConnected(killerid))
	{
		...
		{
		PlayerInfo[killerid][pMats] += 1;
		GivePlayerCash(killerid, 1);
		PlayerInfo[killerid][pCredits] += 1;
		SendClientMessage(killerid, COLOR_YELLOW, "Ban nhan duoc $1 va 1 Credits va 1 vat lieu.");
		}
		SetPVarInt(playerid, "PlayerOwnASurf", 0);
	    #if defined zombiemode
...
public OnPlayerSpawn(playerid)
{
	...
	PlayerTextDrawShow(playerid,KimCuongShow[playerid]);
	TextDrawShowForPlayer(playerid, LOGO_SV[0]);
    if(IsPlayerNPC(playerid)) return 1;
	...
	if(GetPVarInt(playerid, "NGPassenger") == 1)
	{
	    ...
	    TPPlayer(playerid, (X-2.557), (Y-3.049), Z);
	...
	if(InsidePlane[playerid] != INVALID_VEHICLE_ID)
	{
		TPPlayer(playerid, GetPVarFloat(playerid, "air_Xpos"), GetPVarFloat(playerid, "air_Ypos"), GetPVarFloat(playerid, "air_Zpos"));
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(GetPVarInt(playerid, "TruckDeliver") > 0 && gPlayerCheckpointStatus[playerid] != CHECKPOINT_RETURNTRUCK)
	{
		...
		{
			SetPlayerCheckpoint(playerid, 2413.7336,-2137.5742,13.2971, 5);
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			{
			    if(IsPlayerInRangeOfPoint(playerid, 6, 2482.1265,-2125.1733,13.5469) || IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
			    {
				    new vehicleid = GetPlayerVehicleID(playerid);
			...
			{
			    if(!IsPlayerInRangeOfPoint(playerid, 6, 2413.7336,-2137.5742,13.2971) && !IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
				...
				{
				    distancepay = floatmul(GetDistance(2482.1265,-2125.1733,13.5469, TruckerDropoffs[route][PosX], TruckerDropoffs[route][PosY], TruckerDropoffs[route][PosZ]), 1.5);
				}
				...
				if(TruckDeliveringTo[vehicleid] != INVALID_BUSINESS_ID) {
					...
					if(truckdeliver == 5) // Weapons
					{
						if(PlayerInfo[playerid][pConnectHours] >= 2 && PlayerInfo[playerid][pWRestricted] <= 0)
						{
							if(level >= 0 && level <= 50)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Uzi cho viec van chuyen vu khi trai phep.");
							...
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Tec9 cho viec van chuyen vu khi trai phep.");
                                SendClientMessageEx(playerid, COLOR_YELLOW, "* Khong duoc su dung vu khi duoc thuong cho muc dich xau nhu KOS/DM nonRP, neu vi pham se bi han che vu khi.");
					...
					if(truckdeliver == 7) // Illegal materials
					{
						...
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 2500 cho viec van chuyen vat lieu bat hop phap.");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	{
		new Float:X[3];
		if(IsPlayerInRangeOfPoint(playerid, 1, Float:X[0], Float:X[1], Float:X[2]))
		{
   			ShowPlayerDialog(playerid, XEPHANG, DIALOG_STYLE_LIST, "XEP HANG", "TOP 10 - {e600ac}CREDITS{ffffff}\nTOP 10 - {00ff00}MONEY{FFFFFF}\nTOP 10 - {ff9933}ONLINE{FFFFFF}\nTOP 10 - {00ace6}VAT LIEU{FFFFFF}\nTOP 10 - {0000ff}LEVEL{FFFFFF}", "Lua chon", "Huy bo");
        }
	}
    if(newkeys & KEY_YES)
	{
		new Float:X[3];
		GetActorPos(Actorvipsf, Float:X[0], Float:X[1], Float:X[2]);
		if(IsPlayerInRangeOfPoint(playerid, 2, Float:X[0], Float:X[1], Float:X[2]))
		{
			...
			#endif
			{
				return ShowPlayerDialog(playerid, 7483, DIALOG_STYLE_LIST, "VIP Locker", "Mau (Free)\nGiap (Free)\nWeapons\nTrang Phuc\nCong Viec\nVIP Color", "Lua chon", "Huy bo");
				return ShowPlayerDialog(playerid, 7483, DIALOG_STYLE_LIST, "VIP Locker", "First Aid Kit (Free)\nKevlar Vest (Free)\nWeapons\nClothes Corner\nJob Center\nVIP Color", "Lua chon", "Huy bo");
				return ShowPlayerDialog(playerid, 7483, DIALOG_STYLE_LIST, "VIP Locker", "Mau (Free)\nGiap (Free)\nVu Khi\nTrang Phuc\nNghe Nghiep\nVIP Color", "Lua chon", "Huy bo");
			}
		}
    }
	if(newkeys & KEY_SECONDARY_ATTACK)
    {
        ...
        if(IsPlayerConnected(giveplayerid))
        {
            ...
            {
			   if(IsPlayerInRangeOfPlayer(playerid, giveplayerid, 3.0))
			   {
			       if(GetPVarInt(playerid, "lockhoisinh") == 1) return 1;
		       	   if(GetPVarInt(giveplayerid, "lockhoisinh") == 1) return 1;
			       if(gettime() < GetPVarInt(giveplayerid, "hoisinh"))
			       {
			           format(string, sizeof(string), "Ban Phai Cho %d giay moi co the dung lenh len nguoi choi nay.", (GetPVarInt(giveplayerid, "hoisinh") - gettime()));
			           SendClientMessageEx(playerid, COLOR_GREY, string);
			           return 1;
			       }
			       if(gettime() < GetPVarInt(playerid, "hoisinh"))
			       {
			           format(string, sizeof(string), "Ban Phai Cho %d giay moi co the dung lenh nay.", (GetPVarInt(playerid, "hoisinh") - gettime()));
			           SendClientMessageEx(playerid, COLOR_GREY, string);
			           return 1;
			       }
				   SetPVarInt(playerid, "lockhoisinh", 1);
				   SetPVarInt(giveplayerid, "lockhoisinh", 1);
			       SetTimerEx("HoiSinhPlayer", 15000, false, "ii", giveplayerid,playerid);

			       ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1); // Place Bomb
			       format(string, sizeof(string), "[Hoi Sinh]{FFFFFF}Nguoi Choi %s Dang Co Gang Hoi Sinh Cho Ban.", GetPlayerNameEx(playerid));
			       SendClientMessageEx(giveplayerid, COLOR_GREEN, string);
			       format(string, sizeof(string), "[Hoi Sinh]{FFFFFF}Ban Dang Co Gang Hoi Sinh Cho %s.", GetPlayerNameEx(giveplayerid));
			       SendClientMessageEx(playerid, COLOR_GREEN, string);
			   }
            }
        }
    ...
    if(newkeys & KEY_NO)
    {
        if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
       {
            ...
            ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~Khoi Dong Phuong Tien", 3000, 3);
            SendClientMessageEx(playerid, COLOR_GRAD2, "{ff0000}[!]{ffffff} USE: {00FF00}/nangcapxe{ffffff} De Nang Cap Phuong Tien Cua Ban");
	...
	else if(IsKeyJustDown(128, newkeys, oldkeys))
	{
	...
	else if (IsKeyJustDown(KEY_FIRE, newkeys, oldkeys))
 	{
		...
		if(GetPVarInt(playerid, "DraggingPlayer") != INVALID_PLAYER_ID)
		{
			...
			TPPlayer(GetPVarInt(playerid, "DraggingPlayer"), dX, dY, dZ);
...
{
    new string[128];
    if (strcmp("/skema", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Trolling (/skema).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
    if (strcmp("/ct.up", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Trollcar (/ct.up).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/chp", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Vehicle Health (/chp).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/tr", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Slap (/tr).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/kill", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo kill (/kill).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/vb", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Trollcar (/vb).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/cboom", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Trollcar (/cboom).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/hmo", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo Minigun Killer (/hmo).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/invis", cmdtext, true, 10) == 0)
	{
        format(string,sizeof(string),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d) su dung cleo tang hinh (/invis).", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, string, 2);
		return 0;
	}
	if (strcmp("/xprip", cmdtext, true, 10) == 0)
	{
        new String[128];
        format( String, sizeof( String ), "[Laur.] Nguoi choi %s , da bi banned vinh vien do su dung hacking (Kill All).", GetPlayerNameEx(playerid));
		ABroadCast( COLOR_LIGHTRED, String, 2 );
	...
	if (strcmp("/fcrash", cmdtext, true, 10) == 0)
	{
        new String[128];
        format( String, sizeof( String ), "[Laur.] Nguoi choi %s , da bi banned vinh vien do su dung hacking (Kill All).", GetPlayerNameEx(playerid));
		ABroadCast( COLOR_LIGHTRED, String, 2 );
	...
	if (strcmp("/xcrash", cmdtext, true, 10) == 0)
	{
        new String[128];
        format( String, sizeof( String ), "[Laur.] Nguoi choi %s , da bi banned vinh vien do su dung hacking (Kill All).", GetPlayerNameEx(playerid));
		ABroadCast( COLOR_LIGHTRED, String, 2 );
	...
	{
        new String[128];
        format( String, sizeof( String ), "[Laur.] Nguoi choi %s , da bi banned vinh vien do su dung hacking (Trolling).", GetPlayerNameEx(playerid));
		ABroadCast( COLOR_LIGHTRED, String, 2 );
		SendClientMessage(playerid, COLOR_LIGHTRED, String );
		SetPVarInt(playerid, "_HACK_WARNINGS", 0);
		PlayerInfo[playerid][pBanned] = 3;
		new playerip[32];
		GetPlayerIp(playerid, playerip, sizeof(playerip));
		format( String, sizeof( String ), "AdmCmd: %s (IP:%s) da bi khoa tai khoan, ly do: Hack Trolling", GetPlayerNameEx(playerid), playerip);
		PlayerInfo[playerid][pBanned] = 3;
		Log("logs/ban.log", String);
		...
		SystemBan(playerid, "[System] (Hack Trolling)");
		MySQLBan(GetPlayerSQLId(playerid), playerip, "Hack Trolling", 1,"nDPDz");
		SetTimerEx("KickEx", 1000, 0, "i", playerid);
		TotalAutoBan++;
		return 0;
	}
	if (strcmp("/.killall", cmdtext, true, 10) == 0)
	{
        ...
        {
			new String[128];
	        format( String, sizeof( String ), "AdmCmd: %s da bi khoa tai khoan, ly do: MultiHacking.", GetPlayerNameEx(playerid));
			ABroadCast( COLOR_LIGHTRED, String, 2 );
			SendClientMessage(playerid, COLOR_LIGHTRED, String );
			SetPVarInt(playerid, "_HACK_WARNINGS", 0 );
			PlayerInfo[playerid][pBanned] = 3;
			new playerip[32];
			GetPlayerIp(playerid, playerip, sizeof(playerip));
			format( String, sizeof( String ), "AdmCmd: %s (IP:%s) da bi khoa tai khoan, ly do: MultiHacking", GetPlayerNameEx(playerid), playerip);
			PlayerInfo[playerid][pBanned] = 3;
			Log("logs/ban.log", String);
			SystemBan(playerid, "[System] (MultiHacking)");
			MySQLBan(GetPlayerSQLId(playerid), playerip, "MultiHacking", 1,"System");
			SetTimerEx("KickEx", 1000, 0, "i", playerid);
			return 0;
		}
	}
